    ...
    def _val_one_epoch(self, val_data):
        ...
        for step, (data, label) in enumerate(val_data):
            ...
            with t.no_grad():
                ...
                _, prediction = t.max(out.data, dim=1)
        ...
        acc=100*testacc/cm_value.sum()
        print('acc is {}'.format(acc))
        posco=(100. * cm_value[0][0])/(cm_value[0][0]+cm_value[0][1])
        print('pos is {:.3f}'.format(posco))
        negco=(100. * cm_value[1][1])/(cm_value[1][0]+cm_value[1][1])
        print('neg is {:.3f}'.format(negco))
        #print(cm_value)
        cm_value_correct = 0
